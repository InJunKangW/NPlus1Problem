...
@Repository
public class ParentQuerydsl extends QuerydslRepositorySupport {
    private final QParent p = QParent.parent;
    private final QChild c = QChild.child;

    public ParentQuerydsl() {
        super(Parent.class);
    }
    ...
    public List<ChildDto> getChildDtoListWithQuerydsl() {
        return from(c)
            .select(Projections.constructor(ChildDto.class, c.name, p.name))
            .innerJoin(p)
            .on(c.parent.eq(p))
            .fetch();
    }
    ...
    public List<ChildDto> getChildDtoListWithQuerydslWithQueryProjection() {
        return from(c)
            .select(new QChildDto(c.name, p.name))
            .innerJoin(p)
            .on(c.parent.eq(p))
            .fetch();
    }
